...
buche.bss = regsubsets(log(biom)~(1+ I(log(dbh))+I(log(dbh)^2)+I(log(age))+I(log(age)^2)+I(log(height))+I(log(height)^2)+hsl+I(hsl^2)+I(log(hsl))+I(log(hsl)^2))*as.factor(author), data = buche.data, nbest = 3)

## nbest = 3, jeweils die drei besten Modelle gleicher Parameterzahl
# summary(buche.bss)
