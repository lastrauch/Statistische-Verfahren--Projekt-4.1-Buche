...
buche.data = read.csv("buche.csv", sep=",")
head(buche.data)
# Seed fÃ¼r den Random Number Generator
set.seed(0)
...
# 2. alle Teilmodelle sind Kandidatenmodelle
# 3. Entscheidung basierend auf Mallow's Cp
# 4. SchÃ¤tzer fÃ¼r SPSE = Cp * sigma2.max + n* sigma2.max
...
summary(buche.bss.joosten)$cp
summary(buche.bss.joosten)$which[22,] # Modell 22 liefert betragsmÃ¤ÃŸig besten CP-Wert (8.344969)

m.joosten = lm(log(biom)~1+ I(log(dbh))+I(log(dbh)^2)+I(log(age))+I(log(height))+I(log(height)^2)+hsl+I(log(hsl))+I(log(hsl)^2), data = buche.data.joosten)

## Simulation
# Genutzter Datensatz und Anzal der Outputs fÃ¼r Kreuzvalidierung
buche.data.used = buche.data.joosten

# Anzahl der Beobachtungen fÃ¼r den gewÃ¤hlten Datensatz
numberObs = nrow(buche.data.used)
...
for(m in 1:4){
  for(j in 1:reps){
    ...
    # Muessen die simulierten Biomassedaten nicht zu den Parametern passen? Wie wird das erreicht?
    # Wie bekommen wir Simulierte Parameter zu den simulierten Biomassedaten?
    # In einer Loesung von vorherigen Studenten wurden einfach zufÃ¤llig Daten aus der Designmatrix mit den simulierten Biomassedaten verknÃ¼pft. Wieso ist das zielfÃ¼hrend?
    ...
    ## Theoretischer SPSE
    # SPSE = n*sd^2 + (bias ist NULL, da Zufallszahlen Erwartungstreu) + |M|*sd^2
    numberObs.sim = nrow(buche.design)  # Anzahl der Beobachtungen fÃ¼r den gewÃ¤hlten Datensatz
    SPSE.theo[m] = SPSE.theo[m] + n*biom.sd^2 + numberObs.sim*biom.sd^2 
...
>>>>>>> ba89ddf7b92a360573527aedf03449388cd6fde9
